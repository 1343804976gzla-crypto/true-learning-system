...
async def test_long_content():
    ...
    try:
        ...
        for i, q in enumerate(questions, 1):
            ...
            if not options:
                ...
                for opt in ['A', 'B', 'C', 'D', 'E']:
                    ...
                    is_placeholder = (
                        not val or
                        val.strip() == f"（选项{opt}缺失）" or
                        val.strip() == f"选项{opt}（占位符）" or
                        val.strip() == f"选项{opt}"
                    )
                    if opt not in options or is_placeholder:
        ...
        # 保存结果
        with open('quiz_long_content_result.json', 'w', encoding='utf-8') as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        print("✅ 已保存到 quiz_long_content_result.json")
