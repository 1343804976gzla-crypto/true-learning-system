...
from database.domains import CONTENT_DATABASE_URL, CORE_DATABASE_URL, get_sqlite_path
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate content-domain tables into a dedicated content_knowledge.db file.")
    parser.add_argument("--source", type=str, default="", help="Source SQLite database path. Defaults to DATABASE_PATH.")
    parser.add_argument("--target", type=str, default="", help="Target SQLite database path. Defaults to CONTENT_DATABASE_PATH.")
    parser.add_argument("--replace", action="store_true", help="Replace existing rows in the target content tables.")
...
def _ensure_target_schema(target_path: Path) -> None:
    ...
    try:
        ContentBase.metadata.create_all(bind=target_engine)
    finally:
...
def main() -> int:
    ...
    target_path = _resolve_path(args.target, CONTENT_DATABASE_URL)

    if source_path.resolve() == target_path.resolve():
        raise SystemExit("source and target point to the same database file; configure CONTENT_DATABASE_PATH first")

    if not source_path.exists():
        raise SystemExit(f"source database not found: {source_path}")

    source_snapshot = _snapshot_path(source_path, "pre-content-split")
    ...
    if target_path.exists() and args.replace:
        target_snapshot = _snapshot_path(target_path, "pre-content-replace")
    ...
    try:
        ...
        existing_target_rows = sum(_table_count(target_conn, table) for table in CONTENT_TABLES if _table_columns(target_conn, table))
        if existing_target_rows and not args.replace:
            raise SystemExit(
                f"target database already contains {existing_target_rows} content rows; rerun with --replace to overwrite"
            )
    ...
    print(f"[done] content domain migrated to: {target_path}")
    return 0
